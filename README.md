...
The e20_sim_cache.cpp based on simulating the E20 processor based on the E20 manual. The code has the ability to print the configuration of the cache, the log entry (hit or miss in the cache), and contains a main function that parses a file, and configures the cache(s). The main part of the code simulates the use of two caches, storing them and modifying the caches with each access to the cache/memory. I created a separate function that does just this, called cache_fun(). This function is called in the "store word" and the "load word" cases, and it simulates access to and updating the cache.
